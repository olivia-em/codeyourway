...
function setup() {
  ...
  // Stretch video to fit canvas
  video = createCapture(VIDEO);
  video.size(width, height);
  video.hide();

  updateGridSize(); // Adjust grid dynamically
...
function draw() {
  background(0);
  textToDraw = []; // Reset text storage

  let cImage = video.get();
  let videoPixelWidth = cImage.width / pw;  // Width of each video pixel
  let videoPixelHeight = cImage.height / ph; // Height of each video pixel
  let charInput = document.getElementById("i1");
  let char = charInput ? charInput.value : "";

  noStroke();
  cImage.loadPixels();

  // First loop: Draw rectangles & store text positions
  for (let j = 0; j < ph; j++) {
    for (let i = 0; i < pw; i++) {
      let imageI = floor(i * videoPixelWidth);
      let imageJ = floor(j * videoPixelHeight);
      ...
      if (!clearedPixels.includes(`${i},${j}`)) {
        ...
        fill(255);
        textToDraw.push({ char, x, y }); // Store text position for later
        fill(r, g, b);
      }
  ...
  // Second loop: Draw text on top
  fill(255); // Black text color
  textAlign(CENTER, CENTER);
  for (let { char, x, y } of textToDraw) {
    text(char, x + rectSize / 2, y + rectSize / 2);
  }
}

// Adjusts the grid to maintain square pixels
function updateGridSize() {
  let pixelCount = 100; // Base pixel count (adjust as needed)
  
  // Determine square size based on the smaller dimension
  rectSize = min(width / pixelCount, height / pixelCount);

  // Adjust pixel count to fit full width & height
  pw = floor(width / rectSize);
  ph = floor(height / rectSize);
}
